...
public class PathingMain extends PApplet
{
   ...
   */
   ...
   private boolean moveDFS(Point pos, GridValues[][] grid, List<Point> path)
   {
      ...
      ArrayList<Point> neighbors = getNeighbors(grid, pos);

      for(int i = 0; i < neighbors.size(); i++){
         if(moveDFS(neighbors.get(i), grid, path)){
            path.add(0, pos);
            return true;
         }
      }
   ...
   public static ArrayList<Point> getNeighbors(GridValues[][] grid, Point pos){
